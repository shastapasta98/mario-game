...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Press Play to start the totally awesome game -_-", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if(noseX != "" && gameConfig.status==="start" && gameConfig.status==="start") {
    document.getElementById("status").innerHTML = "game has probably loaded"
    world_start.play();
